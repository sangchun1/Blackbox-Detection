...
class DenseTemporalCANHead(nn.Module):
    def __init__(self, input_dim: int = 768, feature_dim: int = 384, hidden: int = 256, layers: int = 2):
        super().__init__()
        ...
        self.heads = nn.ModuleDict({name: nn.Linear(hidden * 2, 1) for name in CAN_TARGETS})

    def forward(self, z: torch.Tensor) -> dict[str, torch.Tensor]:
        # z: B T D
        d1 = torch.cat([torch.zeros_like(z[:, :1]), z[:, 1:] - z[:, :-1]], dim=1)
        d2 = torch.cat([torch.zeros_like(d1[:, :1]), d1[:, 1:] - d1[:, :-1]], dim=1)
...
class VJEPA21DenseCAN(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.head = DenseTemporalCANHead(embed_dim, feature_dim, temporal_hidden, temporal_layers)
    ...
    def _encode(self, video: torch.Tensor) -> torch.Tensor:
        ...
        if not isinstance(outputs, (list, tuple)):
            outputs = [outputs]
        # Normalize each tapped layer independently, then average. This avoids a 4x feature dimension.
        layers = [F.layer_norm(x.float(), (x.shape[-1],)) for x in outputs]
        ...
        if tokens.shape[1] != expected:
            raise RuntimeError(
                f"unexpected V-JEPA token count {tokens.shape[1]}, expected {expected} "
                f"for input {(T, H, W)}"
            )
        z = tokens.view(B, t_tokens, h_tokens * w_tokens, -1).mean(dim=2)
        # Return to 10-Hz frame resolution. Tubelets represent pairs of source frames.
        z = F.interpolate(z.transpose(1, 2), size=T, mode="linear", align_corners=False).transpose(1, 2)
        return z
